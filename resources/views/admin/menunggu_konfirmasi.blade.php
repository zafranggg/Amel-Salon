...
@endsection
    ...
    <!-- Main content -->
      <div class="card-info">
        ...
        @endif
        <h5 class="text-center text-danger fw-bold">Daftar Pembayaran Booking</h5>
        <p class="text-center">Lihat dan bayar pelunasan booking.</p>
       ...
       @include('layout.tabs')
        ...
        <!-- Table Booking Menunggu Konfirmasi -->
                        ...
                        <tr>
                            ...
                            <td>
                              <div class="d-flex gap-2">
                                ...
                                <!-- Di dalam tabel booking -->
                                <button type="button" class="btn btn-success btn-sm"
                                  onclick="openPaymentModal({{ $booking->id_booking }}, {{ $booking->sisa_bayar }})">
                                  Konfirmasi
                                </button>

                                <!-- Tombol Konfirmasi -->
                                {{-- <form method="POST" action="{{ route('booking.konfirmasi', $booking->id_booking) }}">
                                  @csrf
...
<div id="modalDetail{{ $booking->id_booking }}" class="modal">
  <div class="modal-content">
    ...
    <div class="actions">
      ...
      <form action="{{ route('booking.konfirmasi', $booking->id_booking) }}" method="POST" style="display:inline;">
        @csrf
...
<div id="paymentModal" class="modal">
  <div class="modal-content">
    ...
    <form id="paymentForm" method="POST" action="{{ route('booking.selesaikan', $booking->id_booking) }}">
      @csrf
      <input type="hidden" name="id_booking" id="paymentBookingId">
      
      <!-- Tampilan jumlah sisa -->
      <div class="mb-3">
        <label><strong>Jumlah yang Harus Dilunasi:</strong></label>
        <p id="sisaBayarText" style="font-size: 18px; color: red;">Rp -</p>
      </div>
...
<script>
  ...
  function openPaymentModal(bookingId, sisaBayar) {
    document.getElementById('paymentBookingId').value = bookingId;
    document.getElementById('sisaBayarText').innerText = formatRupiah(sisaBayar);
